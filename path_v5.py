...
if not test:
    ...
    remote_devices.append(RemoteXBeeDevice(device, XBee64BitAddress.from_hex_string("0013A200415E8441"))) #full battery
    remote_devices.append(RemoteXBeeDevice(device, XBee64BitAddress.from_hex_string("0013A2004190B158"))) #No Battery
...
def create_miniCs():
    ...
    for i in range(30):
        ...
        cv2.circle(frame, (int(circle_list[i][0]), int(circle_list[i][1])), 20, (255, 0 , 0), 1)
...
# Get desired location of each robot
              ...
              for i in range(len(remote_devices)):
                  ...
                  cv2.circle(frame, (int(desPosition[N[i]][0]), int(desPosition[N[i]][1])), 20, (0, 165, 255), 1)
    ...
    # Check distance from desired location
                  ...
                  if (distanceDes[agent] <= 20.0 and isSuccess[agent] == False): # If distance is less than 30 pixels, then the agent has arrived
                            ...
                            isSuccess[agent] = True # brought back by JYZ
                            pass_var[agent] = True
                            """ #commented out by JYZ
